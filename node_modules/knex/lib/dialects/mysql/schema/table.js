...
TableCompiler_MySQL.prototype.createQuery = function(columns, ifNot, groupedAlterTable) {
  ...
  var uniqueString = "";
  var distributeKey = undefined;
  if(groupedAlterTable) {
      for(var i = 0; i < groupedAlterTable.length; i++) {
          var tmpObj = groupedAlterTable[i];
          if (tmpObj.method !== 'unique')
              continue;
          var argName = tmpObj.args[0];
          uniqueString += ", UNIQUE KEY unique_idx_" + argName + "(`" + argName + "`)";
          if (!distributeKey)
              distributeKey = argName;
      }
  }

  if (distributeKey) {
      var sqlSuffix = ') default character set utf8';
      sql = sql.replace(sqlSuffix, uniqueString + sqlSuffix + ' DISTRIBUTE_KEY(`' + distributeKey + '`)');
  }
