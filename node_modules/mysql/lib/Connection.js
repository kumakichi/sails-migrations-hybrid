...
var sqlLog = (process.env.LOG_QUERIES === 'true') ? console.log : function () {};
...
Connection.prototype.query = function(sql, values, cb) {
  ...
  query.sql = _transformSqlForALI(query.sql);
...
function _transformSqlForALI(sql) {
  if (!sql)
    return sql;
  sqlLog('[before]migrationsxxxxxxxxxxxxxxx:', sql);
  sql = sql.replace('default character set utf8', 'DEFAULT CHARSET=utf8');
  sql = sql.replace('`id` int', '`id` bigint');
  //if (sql.indexOf('create table') >= 0 && sql.indexOf('`id` bigint') >= 0)
  //        sql = sql + ' DISTRIBUTE_KEY(`id`)';

  if (sql.indexOf('`sails_migrations_lock`') !== -1) {
    if (sql.indexOf('create table') >= 0) {
      if(sql.split('`').length === 5)
          sql = sql.replace(')', ', `id` int unsigned not null primary key) DISTRIBUTE_KEY(`id`)');

      if(sql.indexOf('DISTRIBUTE_KEY') === -1) {  //'create table if not exists `sails_migrations_lock` (`is_locked` int)';
        var prefix= sql.replace(/create table .*\(`/,'');
        var firstKey = prefix.split('`')[0];
        sql += ' DISTRIBUTE_KEY(`' + firstKey + '`)';
      }
    }

    if(sql.indexOf('insert into') !== -1) {
      sql = sql.replace('(`is_locked`)', '(`is_locked`, `id`)');
      sql = sql.replace(/(values .*)(\))/,'$1, 1$2');
    }

    if(sql.indexOf('update `sails_migrations_lock` set `is_locked`') !== -1) {
      sql = sql + ' where `id` = 1';
    }
  }
  ...
  sqlLog('[after] migrationsxxxxxxxxxxxxxxx:', sql);
  return sql;
}
